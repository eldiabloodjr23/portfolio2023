...
const Details = ({ position, company, companyLink, time, address, work }) => {
  ...
  return (
    <div
      ref={ref}
      className="my-8 first:mt-0 last:mb-0 w-[60%] mx-auto flex flex-col items-center justify-between mt"
    >
      ...
      >
        <h3 className="capitalize font-bold text-2xl text-cyan-900 ">
          {position}&nbsp;{" "}
          <a
            className="text-cyan-600 capitalize cursor-pointer"
            href={companyLink}
          >
            @{company}
          </a>
        ...
        <div className="font-medium w-full pt-3">{work}</div>
      </motion.div>
    </div>
  );
};
